...
import org.infinispan.server.memcached.configuration.AuthenticationConfiguration;
...
public class Authentication extends BaseRequestProcessor {
   ...
   public static final String HOTROD_SASL_PROTOCOL = "memcached";

   private final MemcachedServerConfiguration serverConfig;
   ...
   public Authentication(Channel channel, Executor executor, MemcachedServer server) {
   ...
   public void authMechList(MemcachedHeader header) {
      writeResponse(header, header.encoder().authMechListResponse(header, server, channel, authenticationConfig.allowedMechs()));
   }

   public void auth(MemcachedHeader header, String mech, byte[] response) {
      if (!enabled) {
   ...
   private void authInternal(MemcachedHeader header, String mech, byte[] response) throws Throwable {
      if (saslServer == null) {
   ...
   private void authComplete(MemcachedHeader header, byte[] serverChallenge) {
   ...
   public Subject getSubject(MemcachedOperation operation) {
