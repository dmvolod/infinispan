...
public class BaseRequestProcessor {
   ...
   private final MemcachedAccessLogging accessLogging;

   BaseRequestProcessor(Channel channel, Executor executor, MemcachedServer server) {
   ...
   void writeException(MemcachedHeader header, Throwable cause) {
      ...
      if (header == null) {
         header = new MemcachedHeader(MemcachedOperation.ERROR, 0, "", 0, (short) 1, 0, MediaType.MATCH_ALL, MediaType.MATCH_ALL);
      } else {
         header.op = MemcachedOperation.ERROR;
      }
   ...
   void writeSuccess(MemcachedHeader header, byte[] result) {
   ...
   void writeSuccess(MemcachedHeader header) {
      writeResponse(header, header.encoder().emptyResponse(header, server, channel, OperationStatus.Success));
   }

   void writeNotExecuted(MemcachedHeader header, byte[] prev) {
      if (header.hasFlag(ProtocolFlag.ForceReturnPreviousValue)) {
   ...
   void writeNotExecuted(MemcachedHeader header) {
      writeResponse(header, header.encoder().emptyResponse(header, server, channel, OperationStatus.OperationNotExecuted));
   }

   void writeNotExist(MemcachedHeader header) {
      writeResponse(header, header.encoder().notExistResponse(header, server, channel));
   }

   protected void writeResponse(MemcachedHeader header, ByteBuf buf) {
      int responseBytes = buf.readableBytes();
      ChannelFuture future = channel.writeAndFlush(buf);
      if (header instanceof AccessLoggingHeader) {
