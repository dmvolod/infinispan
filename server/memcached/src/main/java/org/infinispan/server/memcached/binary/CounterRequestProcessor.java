...
import org.infinispan.server.memcached.binary.counter.listener.ClientCounterManagerNotificationManager;
import org.infinispan.server.memcached.binary.counter.listener.ListenerOperationStatus;
...
class CounterRequestProcessor extends BaseRequestProcessor {
   ...
   private final BiConsumer<MemcachedHeader, StrongCounter> handleStrongGet = this::handleGetStrong;
   private final BiConsumer<MemcachedHeader, WeakCounter> handleWeakGet = this::handleGetWeak;
   private final BiConsumer<MemcachedHeader, StrongCounter> handleStrongReset = this::handleResetStrong;
   private final BiConsumer<MemcachedHeader, WeakCounter> handleWeakReset = this::handleResetWeak;

   CounterRequestProcessor(Channel channel, EmbeddedCounterManager counterManager, Executor executor, MemcachedServer server) {
      super(channel, executor, server);
      this.counterManager = counterManager;
      notificationManager = server.getClientCounterNotificationManager();
   }

   private EmbeddedCounterManager counterManager(MemcachedHeader header) {
      header.cacheName = CounterModuleLifecycle.COUNTER_CACHE_NAME;
      return counterManager;
   }

   void removeCounterListener(MemcachedHeader header, Subject subject, String counterName, byte[] listenerId) {
      executor.execute(() -> removeCounterListenerInternal(header, counterName, listenerId));
   }

   private void removeCounterListenerInternal(MemcachedHeader header, String counterName, byte[] listenerId) {
      try {
   ...
   void addCounterListener(MemcachedHeader header, Subject subject, String counterName, byte[] listenerId) {
      executor.execute(() -> addCounterListenerInternal(header, counterName, listenerId));
   }

   private void addCounterListenerInternal(MemcachedHeader header, String counterName, byte[] listenerId) {
      try {
         writeResponse(header, createResponseFrom(header, notificationManager
               .addCounterListener(listenerId, 0x00, counterName, channel, header.encoder())));
      } catch (Throwable t) {
         writeException(header, t);
      }
   }
